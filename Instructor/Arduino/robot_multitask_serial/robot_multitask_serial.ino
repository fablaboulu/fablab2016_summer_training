...
Scheduler startScheduler = Scheduler();
...
void setup() {
  ...
  Serial.begin(9600);
  while(!Serial);
  Serial.println("Insert the number of seconds to start the robot");
}
...
void loop() {
    if (isSleeping){
      if (serialInputReady){
          ...
          if (time == 0) {
            Serial.println("Please, add a correct number");
         }
          else {
            Serial.println("Waiting " + inputString + " seconds");
            startScheduler.schedule(initRobot,time*1000);
        }
        serialInputReady = false;
        inputString = "";
      }
      startScheduler.update();
    }
    else {
      if (n < 5){
...
void serialEvent() {
  while (Serial.available()) {
    ...
    if (inChar == '\n' || inChar == '\r') {
      serialInputReady = true;
    }
    else if (!isDigit(inChar)){
      inputString = "w";//Mark it with a wrong character
    }
  }
}
